...
- **三态可空布尔（None/False/True）进「计数/约束」时，只有 True 该算，别用 `bool()` 顺带把语义拍平——但也别反过来 fail-closed。** 外援上场上限：`Candidate.borrowed = (is_borrowed_player is True)`。评审第一次要求改成 `is not False`（把 None 当外援、fail-closed），那是**错的**——一个没标过任何人的队会被算成十个外援、永远无解，比漏算管理员没录入的数据更糟。None=未标、False=确认非、True=确认是；计数只认 True（fail-open），None/False 的区别留给显示。哪个方向对取决于「未知」在这个场景意味着什么，不是无脑 fail-closed。（评审 BLOCK 不等于对，读 findings 自己判——这条是 group 3 第二次评审才达成的。）
- **全有或全无的批量写端点，空字符串字段会 422 拖垮整批。** 队伍页批量双打 UTR 走 `PUT /players/current-utr`（一条坏就整批回滚）。清空某个双打输入框若发 `""`，后端 `Decimal("")` 解析失败→422→**所有**改动都没写。清空要发 `null`（=清值），不是 `""`：`doubles_utr: v === "" ? null : v`。与「小数不能按字符串比」同源——手填/清空才触发，导出往返不会。另：这类批量保存的 server action 必须 try/catch 且只在成功时 reset，否则失败静默、脏改动还卡在界面上。
- **改了读新列/新表的后端代码，push 前先确认远程 migration 已手工执行，否则线上一部署就 500。** 远程共享 Supabase 的 migration 是在 Dashboard 手工跑的（见 no-CLI-push 规则），所以「本地 migration 已应用 + 测试全绿」不代表远程有那些列/表。`teams.school_count`、`division_borrowed_limits` 这次就是：后端一 push，Render 部署的新代码读这些列，而远程还没建 → 队伍/阵容页全 500，直到有人去 Dashboard 跑那段 SQL。凡是带 migration 的 change，push 后要么立刻给出远程 SQL 让负责人执行、要么让读路径对缺列/缺表降级（只读旁支才适合降级；核心查询降不了，只能靠先跑 migration）。
- **可空数值的显示 helper 要用 `== null`（松判）而非 `=== null`，否则字段缺失(undefined)会漏过去算出 `NaN`。** 胜率列 `formatWinLoss(wins, losses)` 一开始用 `=== null` 判「从未导入」。真跑页面时满屏 `undefined-undefined` / `NaN%`——根因是**后端进程陈旧**（老 uvicorn 的响应里根本没有这两个字段），前端拿到的是 `undefined` 不是 `null`，`undefined === null` 为 false 于是落到 `undefined-undefined`/除出 NaN。类型契约说 `number | null`，但陈旧/旧版后端会整个省略字段；`== null` 同时兜住 null 与 undefined。与「字段存在不是断言」「本地 uvicorn 不带 --reload 看旧代码」同族——只在真渲染 + 真后端下暴露，jsdom 单测（构造完整对象）看不出。
- **一个 side-effect 有多条写路径时，加护栏要盖住每一条，别只改你手头那条。** 「projected/unrated 双打 UTR 不 mirror 进参赛 UTR」这条门（`_NON_MIRRORING_STATUSES`）当初只加在批量/行内的 `write_current_utrs`，漏了 CSV 的 `apply_sheet`——于是导入一个 projected 双打就无声覆盖了冻结的参赛 UTR。两条路径做同一件「顺带写参赛 UTR」的事，护栏却只在一条上。改这类共享 side-effect 时，先 grep 出所有调用同一 `_mirror_participation`/同一副作用的地方，逐条确认门都在。与「按方法判权不按前缀」同源：护栏要挂在动作的公共点上，不是逐路径靠人记得。
- **新写路径拼出来的值，要过主写路径同样的 DB 约束/校验，否则冒出裸 500。** `saved_lineups.name` 有 `check (char_length between 1 and 60)`，`save_lineup` 在 Python 侧先校验；但 `clone_saved_lineup` 自己拼 `<原名> 副本[N]` 直接 commit，绕过了那道校验——近 60 字的源名一克隆就超长 → `IntegrityError` → 未处理的 500，而不是干净的 4xx。凡是新端点自己构造要落库的值（名字、slug、拼接串），先问「主写路径/DB 对这个字段有什么约束」，把它一并满足（这里是把 base 截断给后缀留位、clamp 到 60）。评审 group 3 第二次才抓到，因为测试没覆盖近上限的源名。
- **本地乐观 state 的「与 props 同步」签名，要盖住所有会被服务器改的显示字段，不只当初引入它的那个。** `SavedLineups` 的 `items` 同步 effect 最早为**重排**写，键取 `saved` 的 **id 顺序**；后来加**改名**（id 不变、name 变）→ 签名不变 → effect 不触发 → 卡片一直显示旧名，看起来「改名失败」其实已落库。键要含所有渲染出来、且服务器可改的字段（`id:name:sort_order`）。与「非受控输入+软导航回填需 key remount」「字段存在不是断言」同族：同步/失效判定的 key 漏一个会变的字段，就会静默显示陈旧值。
- **设密码的 masked 输入必须要两次确认，否则 set-time 手滑存下一个谁都解不开的口令。** scoped-admin-auth 上线后负责人在 `/admin` 设比赛密码，密码框是 `type=password` 遮蔽的、只有一个框，手滑存了个非本意的 hash；之后解锁一律报**通用**的「口令不对」，与真的输错完全一样——没有任何信号说「是设置时打错的」，代码全对却查了半天（先怀疑冷启动、路由未部署、hash 不匹配，逐一排除后才用一个已知简单口令 `test1234` 复现验证是 set-time typo）。修法：`/admin` 加确认框，两次一致才允许提交。一般形式与「错的标签比没有标签更糟」同族：**任何"设置凭据"的遮蔽输入都要 confirm**，且一个"读失败/不匹配"若长得和"输错"一模一样，就是个坏诊断面——让它可区分。
- **撤掉板块级 layout gate 时，原来靠它兜底的每个子路由都要各自补守卫，别只守你改的那页。** player-admin-workbench 把 `players/layout.tsx` 的 `canEdit` 重定向去掉（改成任人可读），只给列表页加了 `EditOnly`/模式开关——但 `merge`/`split`/`unresolved` 三个**只写**子页原来是靠这条 layout 重定向挡住非编辑者的，gate 一撤它们就对任何人可达：写仍被 `adminWrite` 服务端挡住（不是越权），但**不可逆操作的详情泄露给了匿名访问者**，且这些路由没有自己的 `error.tsx`，匿名点提交抛的 `NotLoggedIn` 会冲到全站错误边界而不是就地降级。修法：三个写页各自 `if(!await canEdit) redirect(...)` + 补路由级 `error.tsx`。这是「每条新路由都要自带 error.tsx」和「按方法判权不按前缀」的对偶——**移除**一道共享护栏时，要回头把它盖过的每个点逐一补上，别假设「写被服务端挡住」就等于「这页可以对所有人开」。是 group 3 的迟到评审（评审 subagent 打完分之后才报上来的 HIGH）抓到的，再次印证「评审返回 PASS 不等于评审结束」。
- **整页 gate 用静默 `redirect` 会让用户「点了 A 却跳到 B」，机密页宁可就地渲染锁定态。** opponent-compare 的 `/compare` 未解锁时 `redirect(.../teams)`，用户点「对手对比」直接被弹到队伍页、以为坏了——是用户报上来的。已存阵容确实是机密不能露，但「不露内容」不等于「必须跳走」：改成**留在本页**渲染一条锁定说明 + 就地 `EditModeToggle` 解锁入口，且**不发任何取机密数据的请求**（gate 在 fetch 之前），解锁后 `router.refresh` 进正常视图。一般形式：一个 `canEdit`/登录 gate 要么就地锁定态、要么带明确去向的跳转（并告诉用户为什么），别静默把人弹到一个它没要求去的页面。与「未登录不是死入口」同族，只是这次错在另一头——不是死链接，是无声改道。
- **一份 nav 数据喂多个呈现组件时，改一个 nav 标志要同步改所有消费者的测试。** `nav.ts` 同时喂 `Sidebar` 与 `TopNav`；把「对手对比」从 `pending:true` 点亮成链接时，`Sidebar.test`/`nav.test` 改了、`TopNav.test` 漏了（仍断言禁用），全量跑才红、group-2 评审才抓到。动共享 nav 标志，grep 出所有断言该项状态的测试一起改。
- **本地手工应用 migration，别按 `;` 切分再逐句执行——开头的注释块会把 `set search_path` 那条一起吞掉，表建到 `public`。** player-notes 本地建表时，用「按 `;` split、过滤掉以 `--` 开头的块」跑 migration，结果第一块是**整段注释 + `set search_path to zijing_cup, public`**（`set` 之前没有分号），被当成注释整块丢弃 → search_path 没设 → 表落在 `public`（另一个 app 的 schema），fixture 一 `DELETE FROM zijing_cup.player_notes` 就 `UndefinedTable`。修法：把整个 migration 文件**一次性**交给 `psycopg` 的 `cur.execute(sql)`（一次调用多语句，search_path 在同一连接里对后续 DDL 生效），别自己切句。断言本地（`127.0.0.1`）后照旧。
- **写操作的 `revalidatePath` 只刷了列表页，会漏掉同数据的 `[id]` 深链路由——用 `"layout"` scope 盖住整个段。** player-notes 的 `addPlayerNote`/`deletePlayerNote` 一开始 `revalidatePath(`/${season}/${division}/players`)`（默认 `'page'`，只失效那一条精确路径）；但同一份详情也渲染在 `players/[id]/page.tsx` 这条独立路由上，从深链页追加/删除后时间线陈旧、要手刷才更新。改成 `revalidatePath(`/${season}/${division}/players`, "layout")`——layout scope 会失效该段下所有嵌套路由（列表 `?sel` 与 `[id]` 深链都含）。凡「同一数据在 `/x` 与 `/x/[id]` 两处渲染、写在其一」，revalidate 要挂共同祖先段（layout），不是逐页。group-3 迟到评审抓到。
- **绝对定位的浮层（popover/下拉/tooltip）在 `overflow-hidden` 的祖先里会被裁成一条缝——portal 到 body 用 fixed 定位。** notes-surfacing 的评价弹层最初 `position:absolute` 放在触发元素里；宿主是表格单元格（`Td` 带 `h-10 overflow-hidden`）和 `LineBlock` seat（`overflow-hidden`），弹层被裁到 ~40px 行高，屏幕上只露一条缝——单测（jsdom 无真实布局）与「刚好放得下」的截图都发现不了，是 group-5 验证里用 `getBoundingClientRect` 比祖先 rect 才量出来。修法：`createPortal(panel, document.body)` + `position:fixed` 按触发器 rect 定位，逃开所有 overflow 祖先；「点外部关闭」判断要同时含 `panelRef.contains(target)`（面板不在触发器 DOM 子树里了）。与「`h-screen overflow-hidden` 壳里可变长列表必须自带滚动」同族：**任何要盖在别人上面的东西，先问它的祖先会不会裁它**。
- **给某页「编辑模式」加东西前，先确认编辑模式渲染的是不是同一个组件。** 队伍名单页 `TeamEditPanel` 在查看模式渲染只读的 `RosterTable`，一进编辑模式 `if(!canEdit||!editing){…RosterTable…}` 之后 return 的是它**自己另写的编辑表**（带 UTR 输入、加/移出）——`RosterTable` 根本不在编辑模式的 DOM 里。notes-edit-on-surfaces 第一版把可编辑评价 badge 接在 `RosterTable`，编辑模式下什么都不显示、测试全红，才发现渲染的是另一棵树。修法：把编辑态的控件接到**编辑模式真正渲染的那个组件/表**（这里是编辑表的「队员」单元格），只读态才留在 `RosterTable`。一般形式：一个「view/edit 双态」页往往是两棵组件树，别假设加在只读那棵上编辑态就能看到。
- **hover 打开、又 portal 到 body 的浮层，必须显式做「离开触发器和面板都算离开」的关闭桥，否则会堆叠。** 评价弹层从 absolute 改成 body portal 逃 overflow 裁剪时，顺手把 `onMouseLeave` 关闭删了（portal 后面板不在触发器 DOM 子树里，鼠标从触发器移到面板会先触发 leave，简单 leave-close 会把正要用的面板关掉）——结果 hover 一个个球员，弹层只开不关、糊一屏（用户报的）。修法：触发器与面板各挂 `onMouseEnter=cancelClose`/`onMouseLeave=scheduleClose`（带 ~150ms 桥接延时，进另一个就 cancel），点击则置 sticky（只由点外部/Esc 关）。是用户报的，单测用 `fireEvent.mouseEnter/mouseLeave`+fake timers 复现。
- **hover 打开的浮层在触屏上会「打开就被关」——里面要输入的浮层一律只用点击打开(sticky)、不挂 hover。** 评价弹层做了 hover 打开 + 离开关闭（桌面体验），但真机点小标时浏览器发**模拟 `mouseenter`** 先把弹层以非 sticky 方式打开，随后键盘弹出/首个键入引发的杂散 `mouseleave` 触发延时关闭 → 用户「输入 1 个字符就关窗」。判据：`fireEvent.mouseEnter` 只读弹层应打开、可编辑（含表单）弹层**不应**因 hover 打开；点击打开后 `mouseLeave` + 推进计时器仍在。修法：弹层有 `edit`（可交互）时禁用 hover 开/关，只 `onClick` 打开且 sticky（仅点外部/Esc 关）；纯只读弹层才保留 hover 消散。与「hover 浮层 portal 后要关闭桥」同族——但**能输入的浮层根本不该靠 hover**。真机（`resize_window` mobile + 真实键入）才复现，synthetic 事件与桌面都测不出。
- **给 opsx 评审 subagent 造 group diff 时，别把 `git diff` 的 stderr 合进 diff 文件——CRLF 仓库里 git 的 `LF will be replaced by CRLF` 警告会被 `2>&1` 拼进 diff，评审据此报「文件在某行损坏 / 类型不匹配」的假 BLOCK。** lineup-comments group-2 首轮评审 BLOCK，findings 说 `LineupComments.test.tsx:100` 有 `fireEvent.clicwarning:...k`、`getLineupCommentsBatch` 返回类型不匹配——但源文件干净、`tsc` EXIT 0、vitest 180 全绿。根因：我 `git diff ... > f 2>&1`，git 把每个文件的 CRLF 警告写到 **stderr**，`2>&1` 把它们插进了 diff 正文（正好落在某行 `+` 内容中间），评审读的是被污染的 diff。判据：`grep -c "warning:" <diff>` 应为 0。修法：造 diff 一律 `git diff ... 2>/dev/null > f`（丢 stderr），别 `2>&1`。**评审 BLOCK 先拿 `tsc`/vitest 的真跑结果对一下**——findings 与真跑矛盾就是 diff 假象，不是代码问题（与「评审返回 PASS 不等于评审结束」同族的反面：BLOCK 也不等于真坏）。
- **本机用 curl `-d` POST 中文到后端会 mojibake——e2e 造中文数据走 app 写路径或 Python，别用 shell curl。** lineup-comments 真机 e2e 时用 `curl -d '{"body":"打 THU..."}'` 灌评论，页面上评论体渲染成 `?? THU ????`，一度以为是渲染 bug——但同屏的 placeholder「写一条评论…（如：打 THU…）」中文正常。根因：Windows shell/curl 对 `-d` 里的 UTF-8 中文编码错，DB 存进去就是坏字节，与代码无关。判据：走 app 的追加表单（server action→adminWrite→后端）打的中文渲染正常。造中文测试数据用浏览器真实键入，或 Python `urllib`+`PYTHONIOENCODING=utf-8`，别用 shell curl。与既有「CJK console 乱码是控制台编码不是数据坏」同族——但这次是**写入端**真把数据写坏了。
- **`.mcp.json` 的 `cwd` 被 Claude Code 忽略——stdio MCP 靠 `PYTHONPATH`，别靠 cwd。** utr-import 一直 `CONNECTION_CLOSED`，`/mcp` 只说"Connection closed"，看不出为什么；手动跑 `cd backend && python -m app.mcp_server` 又完全正常，于是误以为是 Application Control 拦截（本机确有那一类前科）。真因在 MCP 日志里：`AppData/Local/claude-cli-nodejs/Cache/<project>/mcp-logs-<server>/*.jsonl` 写着 `ModuleNotFoundError: No module named 'app'`——Claude Code 以**项目根**启动进程，`.mcp.json` 里写的 `cwd: backend` 没有生效。修法：`env.PYTHONPATH` 指向 `backend` 的绝对路径（`cwd` 留着只为别的 MCP 客户端）。两个一般教训：(1) MCP 连不上先读那个 jsonl 日志，里面有子进程的 stderr 原文，别从"手动能跑"反推环境拦截；(2) 复现要照客户端的方式起（项目根 cwd + 真 stdio 管道），用 `Start-Process` 不接 stdin 会因 EOF 立刻退出，看起来像崩溃其实是假象。
- **一个概念在 URL 里若有两种参数写法，读的一侧只认一种——载入/回填就会静默丢。** 排阵 preset：pin 在控件里的 URL 表示是**单座填一人**（`D2a=key`，另一座空），`constraintsFromQuery` 也只按 `${line}a`/`${line}b` 读 pin；但 `buildLoadHref` 却把 pin 写成 `pin=LINE:key`（那是**后端搜索**参数 `getTeamLineups` 的格式）。两种写法并存、读的一侧只认单座 → 载入含 pin 的 preset 时 pin 被静默丢弃，纯 pin 阵「点载入没反应」（lock 一直是单座写法所以没暴露）。是用户报的线上 bug。修法：让 `buildLoadHref` 也写单座 `${line}a=key`，与控件写、`constraintsFromQuery` 读同一种。一般教训：同一语义（pin）在「控件写 URL」「URL 读回」「发后端」三处的编码必须对齐；`buildLoadHref` 的契约本就是"生成与控件完全相同的 query"，pin 那条却偏了。回归测试用 round-trip（构造 href → 按读取侧解析 → 应还原出同样的 pin）最稳。
