...
UTR 一起存下来（排阵结果里每套候选旁的「保存此阵容」，管理员可用）。这支队有一个专门的
**已存阵容页**（`/2026/silver/lineup/<队>/saved`），按**当前** UTR 把每套重判一遍并分四态：
仍合法 / UTR 动了但仍合法（点名谁从多少变到多少）/ 已非法（点名哪条线超了）/ 有人离队。每行
还摆出逐人当前 UTR 与性别、每条线之和与超出量、全队 buffer 用量。合法的那套可一键载入回排阵
页；失效的那套可**就地编辑**——线间互换两人、从名单换个人，每改一次立刻按当前 UTR 重判、就近
告诉你现在合法还是卡在哪条，修好了再存回（存回会重新记下当时的 UTR 快照）。快照只是留档，
永远不会回写去改任何人的参赛 UTR。
