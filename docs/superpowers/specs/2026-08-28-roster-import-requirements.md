...
- 导入 2025 赛季金组与银组的赛前名单后，精确落库：
  - 金组 **6 支球队 / 120 名球员**
  - 银组 **18 支球队 / 339 名球员**（导出的赛前 tab 已包含 `THU-I` 19 人与
    `THU-II` 20 人；表内另有各自的 tab，内容与之重复）

    > 早先本节写的是「13 支球队 / 211 名球员，分散在三个 tab」。那组数字来自一次
    > 被**截断**的表格抓取，只读到 11 支队；由此推出的「5 支球队有排名却无名单」
    > 与「`SJTU` 只有 1 行」两条结论同样是抓取残缺的产物，不是表的问题。
    > 2026-08-28 用总表导出的真实 CSV 复核后更正。
  - `Borrowed Player` 与 `Unrated/Projected/Appeal` 两个伪队名**不出现**在球队表中
- 名单行的唯一键是 `(赛季, 组别, 球队, 姓, 名)`。已核对 2025 全部 459 行：
...
- 评级类别的自动判定可由查询验证：`Rated` 行为「已认证」，`Projected` 行为
  ...
  2025 共 36 行 `Unrated`（金 9 / 银 27），其中 **35 行的 `Notes` 有值**
