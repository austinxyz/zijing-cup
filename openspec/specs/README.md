...
**lineup-page-defaults（2026-09-04）**: 默认视图从「一进来就整解」改成「先看已存阵容、要算再算」。右栏两段：上=该队已存阵容（`CollapsibleSaved` 可折叠、四态重判）、下=候选。候选只在 URL 带 `go` 时算（server 侧门控）；不带 `go` 是草稿（回显控件+已存阵容，不整解），草稿的 roster 走 `getTeamRoster`（无 solve），搜索按钮带隐藏 `go=1`。候选与已存阵容统一「每条线 3 行块」`LineBlock`（行1 线名+和+buffer 占用；行2/3 每人一行 姓名+性别符号 ♂/♀+UTR；五块横排、窄屏折），`--color-male`/`--color-female` token 实测 ≥4.5:1；旧对比表 `CandidateTable`/`CandidateRow` 退役。载入 preset = 预填现有控件、不即搜、可存（覆盖/另存）。就地解锁编辑：页头「编辑模式」开关输 admin 口令，走新 `unlockAdmin`（与 `login` 共用 `authenticate` 核心，只是返 `{ok}` 不 redirect，就地 `router.refresh`）。已存阵容改为**就地编辑**（`编辑` 开 `LineupEditor`，去掉「载入」——把十人当锁定塞回控件无用）；卡头显示队伍**总和**（后端 `SavedLineupOut.total`，Decimal 求和，`player_gone` 为 null）、姓名用「姓 名」。三处交付后修的坑见 CLAUDE.md Pitfalls（载入软导航不刷控件、候选被 `overflow-hidden` 静默裁）。纯前端 + 一处后端响应加字段（无 migration）。
