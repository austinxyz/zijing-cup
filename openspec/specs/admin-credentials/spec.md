...
## Purpose
按比赛（赛季+组别）分发编辑权：每个比赛一个密码，只解锁对应比赛；负责人的密码升级为 super，
解锁全部。此前只有一个全局管理员密码，谁解锁都能改所有比赛，负责人无法把某组的编辑权单独交给该组
队长。密码 hash 存 DB（`salt:hash`，scrypt 在 Next 侧算），后端只存/回字符串、绝不见明文；super
复用现有 `ADMIN_PASSWORD_HASH`（向后兼容，不新增 env）。见 [[admin-access]]（写接口的方法判权底座，
本能力叠在其上）。
