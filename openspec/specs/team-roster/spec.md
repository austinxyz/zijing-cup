...
**查询端读的是队员注册表**（队员 / 赛季参赛 UTR / 队伍成员关系），不是名单快照：
跨年身份先按姓名给一个猜测，再由管理员合并或拆分，所以「这两行是不是同一个人」
有了答案，而快照那套「一名球员在 2025 银组和 2026 金组是两条互不相关的记录」的
前提不再成立。名单的改动走队员管理界面，改完在这里立刻可见。

本能力里描述的 CSV 导入器与 `roster_entries` 快照仍然存在，但**不再被任何读取路径
消费**：它保留为回滚依据与队员迁移命令的输入源，且默认拒绝运行，以免「+N 行」的
输出被读成「网站上会有变化」。
